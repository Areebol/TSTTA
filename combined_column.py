...
def main() -> None:
    ...
    parser.add_argument("--out", default="combined_c.pdf")
    ...
    parser.add_argument("--legend-left-frac", type=float, default=LEGEND_LEFT_FRAC)
    ...
    # 单列自上而下排列，水平居中
    page_w = margin * 2 + cell_w
    page_h = margin * 2 + legend_h + cell_h * 4 + label_h * 4 + gap * 3
    ...
    total_content_w = sum(item_ws)

    # 让图例和第一张图左右对齐
    left_offset = cell_w * max(0.0, min(0.5, float(args.legend_left_frac)))
    content_x_start = margin + left_offset
    if len(legend_items) > 1:
        available_gap = (cell_w - left_offset) - total_content_w
        if available_gap >= 0:
            item_gap = available_gap / (len(legend_items) - 1)
    ...
    for row, (pdf_path, pix) in enumerate(zip(pdfs, pixmaps)):
        # 计算坐标
        x0 = start_x
