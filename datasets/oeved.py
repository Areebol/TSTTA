...
class oeVED(Dataset):
    """
    oVED Dataset:
    1. Columns Reordered: Static (incl SOC) -> Focus Points (One-Hot) -> Dynamic (incl Gradient).
    2. Future Injection: Historical road attributes are REPLACED by future attributes (shifted by seq_len).
    """
    VEHICLE_IDS = ("10", "455", "541")
    DEFAULT_EV_ROOT = Path("./data/segmented_1s_eVED_v9/EV")

    FOCUS_POINTS_MAP = {
        "crossing": 1, "traffic_signals": 2, "stop": 3, "turning_loop": 4,
        "bump": 5, "turning_circle": 6, "motorway_junction": 7, "hump": 8,
        "lift_gate": 9, "gate": 10, "give_way": 11, "bollard": 12,
        "level_crossing": 13, "roundabout": 14, "mini_roundabout": 15, "swing_gate": 16,
    }
    ...
    # Group A: 静态/环境 (Static & Condition)
    STATIC_COLUMNS = [
        "OAT[DegC]",                    
    ...
    # Group C: 动态变量 (Dynamic)
    # * Gradient 被移入此组 (作为核心物理输入) *
    DYNAMIC_COLUMNS = [
        "Gradient Smoothed",            
        "HV Battery Current[A]",        
        "HV Battery Voltage[V]",        
        "Vehicle Speed[km/h]",          # Target
        "Energy_Consumption",           # Target
    ]

    # 最终列顺序: Static -> FP -> Dynamic
    FEATURE_COLUMNS = STATIC_COLUMNS + FP_COLUMNS + DYNAMIC_COLUMNS
    ...
    def __init__(
        ...
        # --- 2. 定义哪些列是 "Future Known" (需要被替换的道路属性) ---
        # 这些列的历史值将被替换为未来值
        # 包含: 海拔, 限速, 路口, 公交站, 所有FP列, 以及 坡度(Gradient)
        base_future_known = [
            "Elevation Smoothed[m]",
            "Speed Limit[km/h]",
            "Intersection",
            "Bus Stops",
            "Gradient Smoothed" # 坡度虽然在Dynamic组，但也属于已知道路信息
        ]
    ...
    def _resolve_ev_root(self) -> Path:
        ...
        if (base / "EV").exists(): return base / "EV"
        if base.name == "EV" and base.exists(): return base
        if self.DEFAULT_EV_ROOT.exists(): return self.DEFAULT_EV_ROOT
        ...
        def _read_trip(p: Path):
            ...
            if self.timeenc == 0:
                stamp_df = pd.DataFrame({
                    "month": dates.dt.month, "day": dates.dt.day,
                    "weekday": dates.dt.weekday, "hour": dates.dt.hour,
                })
                stamp_np = stamp_df.to_numpy()
            else:
                stamp_np = time_features(dates.values, freq=self.freq).transpose(1, 0)

            # 提取数据矩阵 (按 FEATURE_COLUMNS 顺序)
            data_np = df[self.FEATURE_COLUMNS].to_numpy(dtype=np.float32, copy=False)
            
            # --- 核心修改：使用未来已知特征替换当前特征 ---
            # 策略：对于 future_known_idx 中的列，将第 t 行的数据替换为第 t + seq_len 行的数据
            # 这样 Encoder 在 t 时刻看到的“坡度”实际上是未来预测窗口起始点的坡度
            
            L = data_np.shape[0]
            
            # 构造位移索引：向后看 seq_len 步
            # 如果 seq_len=96, 则第 0 行被替换为第 96 行的数据
            shift_steps = self.seq_len
            idx_future = np.arange(L) + shift_steps
            # 边界处理：对于最后 seq_len 个点，它们没有“未来”，直接用最后一个有效值填充 (Clamp)
            idx_future[idx_future >= L] = L - 1
            
            # 提取未来的道路属性
            # shape: [L, Num_Known_Cols]
            future_road_vals = data_np[idx_future][:, self._future_known_idx]
            
            # 执行替换 (In-Place Replacement)
            data_np[:, self._future_known_idx] = future_road_vals.astype(np.float32)
    ...
    def inverse_transform(self, data):
        # Static(8) + FP(16) + Dynamic(5) = 29 cols
        # Dynamic Start = 8 + 16 = 24
        # Dynamic cols: Gradient(24), Curr(25), Volt(26), Speed(27), Energy(28)
        start = 27
        end = 29
        if self.scale == "standard":
