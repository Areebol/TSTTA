...
class oVED(Dataset):
    ...
    def _normalize_data(self):
        # 定义需要排除归一化的列名（分类变量/Flag变量）
        # 0-1编码: Intersection, Bus Stops
        # 1-16编码: Focus Points
        dont_norm_names = ["Intersection", "Bus Stops", "Focus Points"]

        # --- 1. 计算需要排除的列索引 ---
        # A. 原始特征部分 (前 14 维)
        raw_indices = [i for i, name in enumerate(self.FEATURE_COLUMNS) if name in dont_norm_names]

        # B. Future Known 特征部分 (拼接在后面的维度)
        # 逻辑：self._future_known_names 被拼接在原始特征之后
        # 拼接起始位置 = 原始特征数量
        aug_start_idx = len(self.FEATURE_COLUMNS)
        aug_indices = []
        for i, name in enumerate(self._future_known_names):
            if name in dont_norm_names:
                aug_indices.append(aug_start_idx + i)

        # 合并所有排除索引
        no_norm_indices = set(raw_indices + aug_indices)

        # --- 2. 准备数据 ---
        Xtr = self.train.astype(np.float32)
        Xva = self.val.astype(np.float32)
        Xte = self.test.astype(np.float32)

        # 初始化统计参数，默认不做变换 (mu=0, sd=1)
        # 这样对于 no_norm_indices 对应的列，计算 (X - 0) / 1 后值不变
        mu = np.zeros(self.n_var, dtype=np.float32)
        sd = np.ones(self.n_var, dtype=np.float32)
        
        mn = np.zeros(self.n_var, dtype=np.float32)
        denom = np.ones(self.n_var, dtype=np.float32)

        # 创建一个 mask，True 表示需要归一化
        norm_mask = np.ones(self.n_var, dtype=bool)
        if no_norm_indices:
            norm_mask[list(no_norm_indices)] = False

        # --- 3. 执行归一化 ---
        if self.scale == "standard":
            # 仅对 mask 为 True 的列计算均值和标准差
            # 使用 nanmean/nanstd 防止原始数据中有 NaN (虽然前面填过 0.0，但为了稳健)
            valid_mu = np.nanmean(Xtr[:, norm_mask], axis=0)
            valid_sd = np.nanstd(Xtr[:, norm_mask], axis=0)
            
            # 将计算结果填入对应的位置
            mu[norm_mask] = valid_mu
            
            # 处理标准差过小的情况，避免除零
            valid_sd = np.where(valid_sd < 1e-8, 1.0, valid_sd)
            sd[norm_mask] = valid_sd

            # 保存参数 (mu 和 sd 的形状依然是 (n_var,)，方便后续 inverse_transform 兼容)
            self.mu = mu
            self.sd = sd

            # 应用变换
            self.train = (Xtr - mu) / sd
            self.val = (Xva - mu) / sd
            self.test = (Xte - mu) / sd

        elif self.scale == "min-max":
            # 仅对 mask 为 True 的列计算 Min/Max
            valid_mn = np.nanmin(Xtr[:, norm_mask], axis=0)
            valid_mx = np.nanmax(Xtr[:, norm_mask], axis=0)

            mn[norm_mask] = valid_mn
            
            valid_diff = valid_mx - valid_mn
            valid_diff = np.where(valid_diff < 1e-8, 1.0, valid_diff)
            denom[norm_mask] = valid_diff
