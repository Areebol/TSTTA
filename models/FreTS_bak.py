...
class Model(nn.Module):
    ...
    Modified: Frequency domain processing changed from Linear Layer (Matrix Mult) to Element-wise Product.
    """
    ...
    def __init__(self, configs):
        ...
        # --- 修改部分 1：参数形状 ---
        # 原来是 (embed_size, embed_size)，代表全连接矩阵
        # 现在是 (embed_size)，代表逐通道的缩放权重 (Filter)
    ...
    def FreMLP(self, B, nd, dimension, x, r, i, rb, ib):
        # --- 修改部分 2：计算逻辑 ---
        # 之前的 einsum 计算的是 xW (矩阵乘法)
        # 现在的计算逻辑是 x * w (点乘/广播)
        
        # 复数乘法规则: (a + bi) * (c + di) = (ac - bd) + (ad + bc)i
        # x.real = a, x.imag = b
        # r (weight real) = c, i (weight imag) = d
        
        # 广播机制：
        # x.real 形状: [B, nd, dimension // 2 + 1, D]
        # r 形状:      [D]
        # PyTorch 会自动将 r 广播到前面的维度
        
        o1_real = F.relu(
            x.real * r - x.imag * i + rb
        )

        o1_imag = F.relu(
            x.imag * r + x.real * i + ib
        )
        # -------------------------
