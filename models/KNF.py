...
class Model(nn.Module):
  ...
  def __init__(self, cfg, cfg_all):
    super().__init__()
    self.cfg = cfg
    self.input_dim = cfg.input_dim  #! 하나의 segment를 구성하는 time step 수
    input_length = cfg.seq_len
    # input_length = 5
    self.num_steps = cfg.num_steps
    self.latent_dim = cfg.latent_dim
    self.use_revin = cfg.use_revin
    self.regularize_rank = cfg.regularize_rank
    self.use_instancenorm = cfg.use_instancenorm
    self.add_control = cfg.add_control
    self.add_global_operator = cfg.add_global_operator
    self.num_feats = cfg_all.DATA.N_VAR
    self.output_dim = self.input_dim
    num_poly = cfg.num_poly
    num_sins = cfg.num_sins
    num_exp = cfg.num_exp
    ...
    if self.regularize_rank:
      dynamics_params = nn.init.xavier_normal_(
          torch.zeros(self.latent_dim, self.latent_dim))
      self.dynamics_summary = nn.Parameter(dynamics_params)

    ### MLP Encoder: Learning coeffcients of measurement functions ###
    self.encoder = MLP(
        input_dim=self.input_dim * self.num_feats,
        output_dim=(self.latent_dim + self.num_sins * 2) * self.input_dim *
    ...
    if self.add_global_operator:
      self.global_linear_transform = nn.Linear(
          self.latent_dim * self.num_feats + self.len_interas,
          self.latent_dim * self.num_feats + self.len_interas,
          bias=False)

    ### Transformer Encoder: learning Local Koopman Operator ###
    self.encoder_layer = nn.TransformerEncoderLayer(
        d_model=input_length // self.input_dim,
    ...
    self.attention = nn.MultiheadAttention(
        embed_dim=input_length // self.input_dim,
    ...
    if self.add_control:
      ...
      self.control = MLP(
          input_dim=(input_length - self.input_dim) * self.num_feats,
          output_dim=self.latent_dim * self.num_feats + self.len_interas,
          hidden_dim=control_hidden_dim,
          num_layers=cfg.control_num_layers,
          use_instancenorm=self.use_instancenorm,
          dropout_rate=dropout_rate)

    ### MLP Decoder: Reconstruct Observations from Measuremnets ###
    self.decoder = MLP(
        input_dim=self.latent_dim * self.num_feats + self.len_interas,
        output_dim=self.output_dim * self.num_feats,
        hidden_dim=cfg.hidden_dim,
        num_layers=cfg.num_layers,
        use_instancenorm=self.use_instancenorm,
        dropout_rate=cfg.dropout_rate)
