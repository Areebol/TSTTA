...
class Model(nn.Module):
    def __init__(self, configs, conv_kernel=[12, 16]):
        ...
        # [MODIFIED 1]: c_out 改为 configs.d_model，保留高维特征进入 Fused Head
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # MIC 多尺度卷积提取特征，输出维度为 [Batch, Seq_len + Pred_len, d_model]
        dec_out = self.conv_trans(dec_out)

        # ============ [MODIFIED 3: 全局展平与全通道融合投影] ============
        # 1. 截取预测部分：[Batch, pred_len, d_model]
        dec_out = dec_out[:, -self.pred_len:, :]
        B, P, D = dec_out.shape

        # 2. 展平时间与通道特征维度：[Batch, pred_len * d_model]
        dec_out = dec_out.flatten(start_dim=1)

        # 3. 跨时间步和通道进行全局信息融合：[Batch, pred_len * enc_in]
        dec_out = self.global_fused_projection(dec_out)
        dec_out = self.head_dropout(dec_out)

        # 4. 还原为预测目标格式：[Batch, pred_len, enc_in]
