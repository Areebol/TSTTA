...
class Model(nn.Module):
    def __init__(self, cfg):
        ...
        if self.individual:
            ...
            alpha = cfg.alpha
            seed = cfg.seed
            # 注意: 如果 cfg 中没有 dataset_train, 需要在外部确保不报错
            ...
            if self.use_fused_head:
                print(f"Using FusedLinearHead: Linear Channel Interaction Enabled at Output Layer.")
        ...
        if self.individual:
            ...
            for series_idx in range(X.shape[1]):
                ...
                if max_train_N is not None and X_data.shape[0]>max_train_N:
                    ...
                    self.models[series_idx].fit(X_data[idxs].cpu().numpy(), y_data[idxs].cpu().numpy())
                else:
                    self.models[series_idx].fit(X_data.cpu().numpy(), y_data.cpu().numpy())
        else:
