...
class FusedFlattenHead(nn.Module):
    ...
    def forward(self, x): 
        ...
        # Step 1: 展平 Patch -> [Batch, n_vars, nf]
        x = self.flatten_patch(x)
        
        # Step 2: 展平变量维度进行融合 -> [Batch, n_vars * nf]
        x = x.reshape(x.shape[0], -1)
        
        # Step 3: 全局线性映射 (实现 Channel Mixing)
        x = self.linear_fusion(x)
        x = self.dropout(x)
        
        # Step 4: 还原形状 -> [Batch, n_vars, target_window]
        x = x.reshape(x.shape[0], self.n_vars, self.target_window)
        return x

# --- 2. 原始：独立展平输出头 (保留以作对比/回退) ---
...
# --- 3. 基础组件 (EnEmbedding, Encoder, EncoderLayer 保持原样) ---
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.use_fused_head:
            self.head = FusedFlattenHead(self.n_vars, self.head_nf, configs.pred_len, head_dropout=configs.dropout)
        else:
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        enc_out = enc_out.permute(0, 1, 3, 2) # [bs, n_vars, d_model, patch_num+1]

        dec_out = self.head(enc_out) # 如果是 FusedHead，输入会在这里做融合
