...
class FusedLinearProjection(nn.Module):
    ...
    def forward(self, x):
        # iTransformer 会把协变量作为额外的 Token 拼在主变量后面 (形状变成 [B, n_vars + covariates, d_model])
        # 我们在这里先切片，只提取原始的主变量进行融合预测，保证展平维度对齐。
        x = x[:, :self.n_vars, :]
        # x:[B, n_vars, d_model]
    ...
    def __init__(self, configs):
        ...
        if self.task_name in ['long_term_forecast', 'short_term_forecast', 'imputation', 'anomaly_detection']:
            if self.use_fused_head:
                print(f"iTransformerPCD Using FusedLinearProjection: Channel Interaction Enabled for {self.task_name}.")
