...
# 2. 模型与数据集 (原始非PCD模型)
# MODELS=("PatchTST" "MICN" "FreTS" "OLS" "DLinear")
MODELS=("PatchTST")
# 3. 迁移对设置 (Source:Target)
# PAIRS=("ETTh1:ETTh2" "ETTm2:ETTm1")
PAIRS=("ETTh2:ETTh1")
PRED_LENS=(96 192 336 720)

# 3. Dual-TTA 核心参数 (使用 export 确保在 parallel 子进程中可见)
export OFFLINE_LRS=(3e-3 5e-3 1e-3 3e-2 5e-2 1e-4)
...
parallel --lb -j ${TOTAL_JOBS} '
    ...
    RESULT_DIR="./results/0402/Ablation/Offline_ETT/lr_${OFF_LR}/"
    mkdir -p "${RESULT_DIR}"
