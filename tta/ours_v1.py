...
from tta.gating import CILossTrendGating, CGLossTrendGating
    ...
    def adapt_petsa(self):
        ...
        for idx, inputs in enumerate(self.test_loader):
            ...
            while batch_end < len(enc_window_all):
                ...
                batch_idx += 1
                pred_rwa, gt_raw = forecast(self.cfg, inputs, self.model, self.norm_module)
                with torch.no_grad():
                    if isinstance(self.cali.gate_mod, CILossTrendGating):
                        se = F.mse_loss(pred, ground_truth, reduction='none')
                        per_channel_mse = se.mean(dim=(0, 1)) # 形状为 [n_vars]
                        self.cali.gate_mod.update_loss(per_channel_mse)
                    elif isinstance(self.cali.gate_mod, CGLossTrendGating):
                        mse_loss = F.mse_loss(pred, ground_truth)
                        self.cali.gate_mod.update_loss(mse_loss)
...
class Calibration(nn.Module):
    def __init__(self, cfg):
        ...
        self.gate_mod = CILossTrendGating(self.n_var, window_size=48, s_max=1.0)

        if cfg.MODEL.NAME == 'PatchTST':
            self.in_cali = GCM(self.seq_len, 1, self.gate_mod, self.var_wise, self.low_rank)
            self.out_cali = GCM(self.pred_len, 1, self.gate_mod, self.var_wise, self.low_rank)
        else:
            self.in_cali = GCM(self.seq_len, self.n_var, self.gate_mod, self.var_wise, self.low_rank)
            self.out_cali = GCM(self.pred_len, self.n_var, self.gate_mod, self.var_wise, self.low_rank)
    ...
    def update_loss_history(self, raw_loss_tensor):
        """
        供 Adapter 调用，更新门控的历史 Loss 趋势
        """
        # 如果 raw_loss_tensor 是标量，扩展到各 channel
        if raw_loss_tensor.dim() == 0:
            raw_loss_tensor = raw_loss_tensor.expand(self.n_var)
        self.gate_mod.update_loss(raw_loss_tensor)